...
# consts
no_request_body_set = frozenset({201, 204, 308})
...
@api.put("/video", response={no_request_body_set: None}, url_name='put_video')
...
@api.post("/authorize", response={404: PureText})
def post_user_athorize(request, user_info: UserLogin):
    """
    route for user authorizing
    """
    user = authenticate(  # django cookie base auth
        request,
    ...
    if user is not None:
        ...
        return 404, {"status": "user or password incorrect"}
    else:
